...
:exclamation: 1. Install aws-cli and serverless globally

```
npm install -g aws-cli serverless
```

:exclamation:2. Then create a user in AWS and create a secret key and access key for the same user.

:exclamation:3. There are variety of cloud platforms that serverless can connect to like Google Cloud platform, AWS etc. To make serverless know what we need to use Amazon web services use the command below.
...
:exclamation: 4. Then setup a serverless files for nodejs using a default boilerplate. Using the command below you can create a severless boilerplate template. The below syntax will create 3 files
